...
- (void)setModel:(DRShowCommentModel *)model
{
    ...
    [self.commentLabel onTapRangeActionWithString:nickNames tapClicked:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (index == 0) {
            ...
            showDetailVC.userId = _model.userId;
            showDetailVC.nickName = _model.userNickName;
            showDetailVC.userHeadImg = _model.userHeadImg;
            [self.viewController.navigationController pushViewController:showDetailVC animated:YES];
        }else
        {
            DRUserShowViewController * showDetailVC = [[DRUserShowViewController alloc] init];
            showDetailVC.userId = _model.toUser.id;
            showDetailVC.nickName = _model.toUser.nickName;
            showDetailVC.userHeadImg = _model.toUser.headImg;
            [self.viewController.navigationController pushViewController:showDetailVC animated:YES];
        }
    }];
}
