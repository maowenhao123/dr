...
+ (instancetype)defaultConfig {
    ...
    config.topToolBarHiddenType    = HiddenNever;
...
/**播放完成回调*/
@property (nonatomic, copy) void (^FullChangeBlcok) (void);
...
- (void)updateWithConfig:(void(^)(CLPlayerViewConfig *config))configBlock {
    ...
    self.maskView.bottomToolBar.hidden    = self.config.hiddenToolBar;
    [self resetToolBarDisappearTime];
    [self resetTopToolBarHiddenType];
    [self resetExitButtonHiddenType];
}
...
//MARK:JmoVxia---重置退出按钮隐藏方式
-(void)resetExitButtonHiddenType{
    switch (self.config.exitButtonHiddenType) {
        case HiddenNever:
            //不隐藏
            self.maskView.exitButton.hidden = NO;
            break;
        case HiddenAlways:
            //小屏和全屏都隐藏
            self.maskView.exitButton.hidden = YES;
            break;
        case HiddenSmall:
            //小屏隐藏，全屏不隐藏
            self.maskView.exitButton.hidden = !self.isFullScreen;
            break;
        case HiddenFull:
            //小屏不隐藏，全屏隐藏
            self.maskView.exitButton.hidden = self.isFullScreen;
        ...
        case UIGestureRecognizerStateBegan:{ // 开始移动
            ...
            if (x > y) { // 水平移动
                ...
                [UIView animateWithDuration:0.5 animations:^{
                    ...
                    self.maskView.exitButton.alpha    = 1.0;
...
-(void)cl_fullButtonAction:(UIButton *)button{
    if (!_isFullScreen){
        [self fullVideoPlayer];
    }else{
        [self originalscreen];
    }
    if (self.FullChangeBlcok){
        self.FullChangeBlcok();
    }
    //重新添加工具条定时消失定时器
    [self resetToolBarDisappearTime];
}
- (void)fullVideoPlayer
{
    _isUserTapMaxButton = YES;
    [self fullScreenWithDirection:UIInterfaceOrientationLandscapeLeft];
}
//MARK:JmoVxia---退出按钮响应事件
- (void)cl_exitButtonAction:(UIButton *)button
{
    [self pausePlay];
    if (self.BackBlock){
        self.BackBlock(nil);
...
- (void)disappearAction:(UIButton *)button{
    ...
    if (!_isDisappear){
        [UIView animateWithDuration:0.5 animations:^{
            ...
            self.maskView.exitButton.alpha    = 0;
...
- (void)fullChange:(FullChangeBlcok)fullChange
{
    self.FullChangeBlcok = fullChange;
}
//MARK:JmoVxia---暂停播放
- (void)pausePlay{
    self.maskView.playButton.selected = NO;
